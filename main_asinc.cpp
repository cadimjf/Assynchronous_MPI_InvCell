...
int main( int argc, char *argv[] ) {
    ...
    printf("apos calcular o rank\n");
    nprocs = MPI::COMM_WORLD.Get_size(); 
    printf("apos achar num processos\n");
	...
	printf("antes de enviar============\n");
        ...
        for(int gen = 1; gen < numGens; gen++) {       	
            ...
            while(rcvInd < nInd) {			
                int index;
                fit = receiveFitness();
                printf("2 recebendo individuo %d\n", rcvInd);
                index = (int)fit[0];
                pop.setIndFitness(rcvInd,fit[1]);
                rcvInd++;
                if (count<nInd) {
		...
		if(gen==numGens-1){
		         for(int slave = 1; slave <= nSlaves; slave++) {
        			printf("enviando msg de morte %d\n",slave);
        	        	sendIndividual(pop.getIndividual(0) , -1, slave);
        	    	}
       		}
       }			
    ...
    printf("ates de abortat\n");
//    MPI::COMM_WORLD.Abort(0);
    printf ("drepois de abortar");
}else {
        int p = 0;
